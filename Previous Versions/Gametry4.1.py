    ...
    def __init__(self, platPos_x, platPos_y, platPos_w):
        ...
        for n in range(len(platPos_x)):
            self.platforms.append([platPos_x[n],
                                   ...
                                   True])
        for p in platPos_y:
            if p+16 not in platPos_y:
                self.freePlatforms.append(p)
...
class Lemmings:
    ...
    def update_lemming(self):
        for s in range(len(self.lemming_x)):
            ...
            if not self.lemming_umbrella[s]:
                if not self.lemming_right[s]:
                    self.lemming_x[s] = self.lemming_x[s] - .5
    
                if self.lemming_right[s]:
                    self.lemming_x[s] = self.lemming_x[s] + .5
            elif self.lemming_umbrella[s]:
                if self.lemming_fall[s]:
                    self.lemming_x[s] += .1
                elif not self.lemming_fall[s]:
                    self.lemming_x[s] += .66
            ...
            if self.lemming_y[s] > pyxel.height and not self.lemming_umbrella[s]:
...
class Umbrella:
    def __init__(self, cursor: Cursor):
        self.x = cursor.cursor_x
        self.y = cursor.cursor_y
...
class Game:
    ...
    def update(self):
        ...
        if pyxel.frame_count % 20 == 0 and self.lemmingCounter < self.lemmingsvar.lemming_number:
            ...
            self.lemming_umbrella.append(False)
            self.lemmingCounter += 1

        if pyxel.btnp(pyxel.KEY_W):
            self.umbrellas.append(Umbrella(self.cursor))

        for u in range(len(self.umbrellas)):
            for l in range(len(self.lemming_x)):
                if self.lemming_y[l] == self.umbrellas[u].y and self.lemming_x[l] == self.umbrellas[u].x:
                    self.lemming_umbrella[l] = True
        ...
        if pyxel.btn(pyxel.KEY_RIGHT) and self.cursor.cursor_x < pyxel.width - 16:
            self.cursor.cursor_x += 16
        if pyxel.btn(pyxel.KEY_LEFT) and self.cursor.cursor_x > 0:
            self.cursor.cursor_x -= 16
        if pyxel.btn(pyxel.KEY_UP) and self.cursor.cursor_y > 0:
            self.cursor.cursor_y -= 16
        if pyxel.btn(pyxel.KEY_DOWN) and self.cursor.cursor_y < pyxel.height - 16:
            self.cursor.cursor_y += 16
        ...
        if is_active:
            for g in range(len(self.lemming_x)):
                if (
                    ...
                    if self.lemmingfell[g] and pyxel.frame_count > 20 and not self.lemming_umbrella[g]:
                        self.lemming_is_alive[g] = False
                        self.deathtime.append(time.time())
    
                elif (self.lemming_y[g] + 16 >= y and self.lemming_y[g] + 16 <= y + 16):
                    if(self.lemming_x[g]+16 < x or self.lemming_x[g] > x+length):
    ...
    def draw(self):
        ...
        pyxel.text(59, 3, "Alive: %i"%(self.lemming_is_alive.count(True)), 7)
        ...
        for h in range(len(self.lemming_x)):
            if self.lemming_is_alive[h]:
                if not self.lemming_umbrella[h]:
        ...
        pyxel.blt(self.cursor.cursor_x, self.cursor.cursor_y, 2, 16 if pyxel.frame_count % 30 < 15 else 0 , 48, 16, 16, colkey=0)
        for u in range(len(self.umbrellas)):
            pyxel.blt(self.umbrellas[u].x, self.umbrellas[u].y, 2, 16 * animate , 16, 16, 16, colkey=0)
